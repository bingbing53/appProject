<template> 
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span style="margin: 5px; margin-left: 25px; float: right;">现金账户余额：{{balance}}</span>
      <el-button
        class="btn-add"
        @click="withdrawCashApply()"
        size="mini">
        提现申请
      </el-button>
    </el-card>
...
<script>
  import {fetchList, shBalance, isExistApply} from '@/api/shopAccount'
  ...
  export default {
    ...
    methods: {
      ...
      async withdrawCashApply() {
        //是否存在已提交的流程
          await isExistApply().then(response => {
              let {data} = response
              if(data || data != null) {
                  this.$message({
                      message: '有未处理的提现申请！',
