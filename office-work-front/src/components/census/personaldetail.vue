...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.idcard != undefined) {
      ...
      if (localStorage.getItem("idcard")) {
        this.keyword = localStorage.getItem("idcard");
        if (this.keyword != "") this.getPersonData(this.keyword);
      }
    }
  },
  activated() {
    ...
    if (
      window.localStorage.getItem("address_strs") != null &&
      window.localStorage.getItem("address_strs") != ""
    ) {
      that.userData.presentDetailAddr1 = window.localStorage.getItem(
        "address_strs"
      );

      that.userData.address = window.localStorage.getItem("addresss"); //其实是找不到部分的详细地址
      that.userData.detailAddress = that.userData.presentDetailAddr1;
      var itemArray = JSON.parse(
        window.localStorage.getItem("address_arrdatas")
      );
      console.log(itemArray, "itemArray");
      //其实是个string
      //   that.userData.buildingId = itemArray.split(",")[2];
      //   that.userData.unitNo = itemArray.split(",")[3];
      //   that.userData.houseNumber = itemArray.split(",")[4];
      that.userData.buildingId = itemArray[2];
      that.userData.unitNo = itemArray[3];
      that.userData.houseNumber = itemArray[4];
      //   window.localStorage.setItem("address_strs", "");
      //   window.localStorage.setItem("addresss", "");
      //   window.localStorage.setItem("address_arrdatas", "");
    }
  },
  ...
  methods: {
    ...
    async getPersonData(keywords) {
      ...
      if (localStorage.getItem("isback") == "true") {
        ...
        var itemArrays = JSON.parse(
          window.localStorage.getItem("address_arrdatas")
        );
        this.userData.buildingId = itemArrays[2];
        this.userData.unitNo = itemArrays[3];
        this.userData.houseNumber = itemArrays[4];
    ...
    saveInfo() {
      console.log(this.userData.address, "this.userData.address");
      console.log(this.userData.detailAddress, "this.userData.detailAddress");
      var data = {
    ...
    clickToSelectAddress() {
      ...
      window.localStorage.setItem("idcard", this.keyword);
      this.$router.push({
        path: `/personalAddress`,
        query: { idcard: this.keyword },
      });
    },
  },
};
