    ...
    <div  >
      ...
      <div>
        <div class="photo" @click="uploadIDCards(0)">
           <div v-if="imageArr.length > 0" v-for="(item,index) in imgList" :key="index" > 
      ...
      <div>   
        <div class="photo"  @click="uploadIDCardss(0)" >
           <div  v-if="imageArr.length > 0" v-for="(item,index) in imgLists" :key="index"> 
        ...
        methods:{
         uploadIDCards(value){        
          ...
          //   }    
            ...
            //       return;
            //     })
            //     var result=this.imgLists;
            //     if(result.status==200){
            //       console.log(status);               
            //       resultArr1.push(result.data["url"]);
            //     }
            //   }
            // }    
            //   wx.closeLoading();
            //   this.imgLists=JSON.stringify(resultArr1); 
            //   this.$f7router.navigate(`/estate/applyDoorAuth`); 
          }
        }
      }
