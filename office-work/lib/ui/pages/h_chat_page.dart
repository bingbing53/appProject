...
class ChatPage extends StatelessWidget {
  ...
  Widget buildContactUser(BuildContext context, List<ContactUserModel> list,WorkBloc bloc) {
...
//                            errorWidget: (context, url, error) => new Icon(Icons.person,color: Colors.white,),
  ...
  Widget _buildHeader(context,bloc) {
    return Column(
      children: <Widget>[
        GestureDetector(
          onTap: (){
            NavigatorUtil.pushPage(context, MessagePage(labelId: Ids.titleMessage,showGroup: "1",),pageName: '议事厅');
          },
          child: Row(
            children: <Widget>[
              Container(
              ...
              Expanded(
                child: Text('议事厅',style: TextStyle(fontSize: Dimens.font_sp16),),
              )
            ],
          ),
        ),

        Divider(height: 1.0,color: Colours.divider,),
          ...
          children: <Widget>[
            ...
            Expanded(
              child: GestureDetector(
                child: Text('组织机构',style: TextStyle(fontSize: Dimens.font_sp16),),
                onTap: (){
                  NavigatorUtil.pushPage(context, ChatGroupPage(labelId: '',),pageName: '组织机构');
                },
              )
            )
          ],
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            Expanded(
              child: GestureDetector(
                child: Text('我的小组',style: TextStyle(fontSize: Dimens.font_sp16),),
                onTap: (){
                  if(hasPermison){
                    NavigatorUtil.pushWeb(context,title: '我的小组',url:WanAndroidApi.MYGROUP);
                  }
                },
              )
            )
  ...
  Widget iconModule(bloc){
    return StreamBuilder(
        ...
        builder: (BuildContext context,
            AsyncSnapshot<List<ModuleModel>> snapshot) {
          return IconButton(icon: new Icon(
              ...
              if(result == 'addGroup'){
                ...
                NavigatorUtil.pushPage(context,
                    new BlocProvider<MissionBloc>(
                      child:new ChatCreateGroup(),
                      bloc:new MissionBloc()
                    ),
                    pageName: Ids.titleChatMessage);
              }else if(result == 'invite'){
                NavigatorUtil.pushPage(context, new InvitePerson(),pageName: Ids.titleInvitePerson);
              }else if(result == 'groupAdd'){
                //新建社工小组
                for(var i = 0;i<snapshot.data.length;i++){
                  if(snapshot.data[i].superCode == 'chat' && snapshot.data[i].code == 'groupAdd'){
                    NavigatorUtil.pushWeb(context,title: snapshot.data[i].showName,url:snapshot.data[i].target);
                    break;
                  }
                }
              }
