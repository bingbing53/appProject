...
class LoginBloc implements BlocBase {
  ...
  Future login(context,{username,password,type,code,invite,address,idcard}) async{
    return wanRepository.login(username: username,password: password,type: type,code: code,invite: invite,address: address,idcard: idcard).then((result){
      List userOrg = result['userOrg'];
      ...
      if(userOrg.length == 1){
        ...
        if(result['restPasswd'] == 1){
          ...
          _loginSuccessSink.add(result['token']);
        }
      }else{
        List<String> choises = [];
        for(var i = 0;i<userOrg.length;i++){
            choises.add(userOrg[i]['departname']);
        }
        CommonUtils.showModalActionSheet(context, "请选择部门", choises, (index){
          bindOrg(context,result['token'],userOrg[index]['orgCode'],result['random']);
        });
      }
    }).catchError((_){
      print(_);
