<template>
  <div class="bgcw2">
    <div class="pd-15">
      ...
      <div class="c-999 f-12 mt-10 pt-10 record pr-10 "  v-if="article.articleType == 0">
      ...
      <div v-if="articleReadList.length > 0">
        <div class="read_box pt-10 pb-10 pl-10" v-for="(item,index) in articleReadList">
          ...
          <div class="read_text pl-10">
            <p class="mb-5">
              <span class="f-15 mr-5">{{item.userName}}</span>
              <van-tag plain size="mini" type="primary" v-if="item.isRead == 1" class="v_middle">已读</van-tag>
              <van-tag plain size="mini" v-if="item.isRead == 0" class="v_middle">未读</van-tag>
            </p>
            <p class="f-12 mt-8 c-999" v-if="item.readTime">阅读时间： {{item.readTime}}</p>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getDetail() {
      ...
      this.articleReadList = res.read;
      try {
          this.article.createDate = this.article.createDate.substring(0, 10);
      }catch (e) {
      ...
      if(this.article.articleType == 1){
          let json = JSON.parse(this.article.images);
          this.article.video = this.common.showPicture(json.video,"",false);
          this.article.imageArr =  this.common.showPicture(json.img,"",false);
      }
      if(!!this.articleReadList && this.articleReadList.length > 0){
          for(var i = 0;i<this.articleReadList.length;i++){
              if(!!this.articleReadList[i].portrait)
                this.articleReadList[i].portrait = this.common.showPicture(this.articleReadList[i].portrait,"",false);
          }
      }
    },
  },
