  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="coupon"
      ...
      <el-form-item label="优惠券名称：" prop="name">
        <el-input v-model="coupon.name" class="input-width" maxlength="12"></el-input>
      </el-form-item>
      ...
      <el-form-item label="面额：" prop="amount">
        <el-input v-model.number="coupon.amount" placeholder="面值只能是数字，限2位小数" class="input-width">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item label="每人限领：">
        <el-input v-model="coupon.perLimit" placeholder="只能输入正整数" class="input-width" :disabled="true">
          <template slot="append">张</template>
        </el-input>
      </el-form-item>
      <el-form-item label="使用门槛：" prop="minPoint">
        <el-input v-model.number="coupon.minPoint" placeholder="只能输入数字" class="input-width">
          <template slot="prepend">满</template>
          <template slot="append">元可用</template>
        </el-input>
        <p>默认值为0，表示无使用门框。大于0则表示有使用门框。</p>
      </el-form-item>
      <el-form-item label="有效期：">
        <el-date-picker type="date" placeholder="选择日期" v-model="coupon.startTime" @change="startTimeChange" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 150px"></el-date-picker>
        <span style="margin-left: 20px;margin-right: 20px">至</span>
        <el-date-picker type="date" placeholder="选择日期" v-model="coupon.endTime" @change="endTimeChange" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 150px"></el-date-picker>
        <p>起止日期均为当日0点0分0秒（例如：2020-03-27至2020-04-28，表示有效期的开始日期为3月27日0点0分0秒，结束日期为4月28日0点0分0秒）。</p>
        <p>代金券的有效期建议至少7天以上。</p>
      </el-form-item>
      ...
      <el-form-item v-show="coupon.useType===2" label="可使用商品：">
      <!-- <el-form-item v-show="coupon.useType===2"> -->
        ...
        <p>输入关键词，搜索商品名称/商品货号，在弹出的搜索结果下拉框里，选中商品后，点击“添加”按钮。</p>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          amount: [
            {type: 'number', required: true, message: '面值只能是数字，限2位小数', trigger: 'blur'}
          ],
          minPoint: [
    ...
    methods:{
      ...
      endTimeChange(val) {
        this.coupon.endTime = val;
      },
      buildCouponParam() {
        var couponParam = Object.assign({}, this.coupon);
        couponParam.startTimeString = couponParam.startTime;
        couponParam.endTimeString = couponParam.endTime;
        couponParam.startTime = null;
        couponParam.endTime = null;
        return couponParam;
      },
      onSubmit(formName) {
        if (this.coupon.productRelationList.length == 0) {
          this.$message({
            message: '可使用商品不能为空，至少选择一个商品！',
            type: 'error',
            duration:1000
          });
          return;
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              if(this.isEdit){
                updateCoupon(this.$route.query.id, this.buildCouponParam()).then(response=>{
                ...
                createCoupon(this.buildCouponParam()).then(response=>{
                  this.$refs[formName].resetFields();
                  this.$message({
                    message: '提交成功',
                    type: 'success',
