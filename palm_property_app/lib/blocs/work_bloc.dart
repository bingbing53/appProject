...
import 'package:estate/utils/download_utils.dart';
import 'package:estate/utils/xpush_utils.dart';
...
import 'package:estate/db/db_model.dart';
import 'package:estate/event/event-bus.dart';
import 'package:estate/ui/dialog/simple_dialog.dart';
import 'package:lpinyin/lpinyin.dart';
import 'package:jmessage_flutter/jmessage_flutter.dart';

const String kMockAppkey = 'eb96f1250b8f12c15f46e491'; //物业端key
const String shequejiaAppkey = '247b688324c3363a535d5bdd';//社区一家key
const String officeAppkey = 'f3ec3edba9f954c719c2a36a';//掌上社区key
...
class WorkBloc implements BlocBase {
  ...
  BehaviorSubject<Map<String,JMUserInfo>> _jmUserInfo = BehaviorSubject<Map<String,JMUserInfo>>();

  Sink<Map<String,JMUserInfo>> get jmUserInfoSink => _jmUserInfo.sink;

  Stream<Map<String,JMUserInfo>> get jmUserInfoStream => _jmUserInfo.stream;
  ...
  ///组织机构
  BehaviorSubject<SysUserModel> _sysUserInfoRepos =  BehaviorSubject<SysUserModel>();
  Sink<SysUserModel> get _sysUserInfoReposSink => _sysUserInfoRepos.sink;
  Stream<SysUserModel> get sysUserInfoReposStream => _sysUserInfoRepos.stream;
  /// 组织机构
...
//        break;
  ...
  //获取我自己的信息
  Future getMineInfoData(){
    estateRepository.getMineInfoData().then((val){
      _sysUserInfoReposSink.add(val);
    });
  }
...
//    }
  ...
  //图片选择
  Future<File> imagePickerImage(type) async{
...
//  ScrollController _scrollController;
  ...
  //停止录音
  void stopRecorder({Map<String,String> extras}) async{
    if(!flutterSound.isRecording)
      return;
    try {
      String result = await flutterSound.stopRecorder();
      print('stopRecorder: $result');
      sendVoiceMessage(recorderPath,extras:extras);
    } catch (err) {
...
//      Fluttertoast.showToast(msg: '停止音乐');
  ...
  //发送文字消息
  Future sendMessage(_textEditController,model,{Map<String,String> extras}) async{
  ...
  //发送图片消息
  Future sendImageMessage(model,{Map<String,String> extras}) async{
    await jiguangLogin();
  ...
  //发送语音消息
  Future sendVoiceMessage(String path,{Map<String,String> extras}) async{
    await jiguangLogin();
    if(Platform.isIOS)
...
//          map.putIfAbsent('type',type as dynamic);
  ...
  void userRegister() async{
    //先通过token获取个人基本信息
    userInfoModel = await estateRepository.getMineInfoData();

    print('---------------------------');
    print(userInfoModel.avatar);
    //注册用户
    try {
      await jmessage.userRegister(username: userInfoModel.phone,
          password: Utils.generateMd5(userInfoModel.phone),
          nickname: userInfoModel.realname);
    }catch(_){
      print(_);
    }finally {
      //存储手机号
      SpUtil.putString(Constant.key_login_name,userInfoModel.phone);

      try {
        await jmessage.login(username: userInfoModel.phone,
            password: Utils.generateMd5(userInfoModel.phone));
        await jmessage.updateMyInfo(gender: JMGender.female,
            nickname: userInfoModel.realname,
            extras: {
              "portrait": userInfoModel.avatar,
            });
      }catch(_){
        print(_);
      }
  ...
  Future enterConversation(JMConversationInfo conversationInfo,{String appkey = kMockAppkey}){
    ...
    switch(conversationInfo.conversationType){
      case JMConversationType.single:
        jmessage.enterConversation(target: getJMSingle(conversationInfo.target.username,appkey:appkey));
        break;
  ...
  Future exitConversations(){
      this.exitConversation(_conversationInfo);
  }
  //删除聊天会话，同时也会删除本地聊天记录。
...
//    getAllUnreadCount();
  ...
  Future addListener(SysUserModel userInfoModel) async {
    LogUtil.e('添加消息监听');
