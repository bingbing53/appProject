<template>
  <f7-page>
    <div>
      ...
      <div>
        <div class="photo" @click="uploadIDCards(0)" v-if="imgList.length>0">
           <div v-for="(item,index) in imgList" :key="index"> 
            <img :src="item"  @click="imgs(index)"  width="210" height="145" />
           </div> 
           <div >        
            <i class="aui-iconfont aui-icon-camera cameraPicture" style="display:none;">+</i>
            <div class="tip">点击拍照/上传人像面</div>
           </div>
        </div>
      </div>
      <div>   
        <div class="photo"  @click=" uploadIDCards()">
           <div v-for="(item,index) in imgLists" :key="index"> 
            <img :src="item"  @click="imgs1(index)" width="210" height="145"/>
           </div> 
