...
//  }
  ...
  static bool sameDay(DateTime day1,DateTime day2){
  ...
  //获取两个时间差
  static String getTimeDiff(DateTime time,String status){
    if(status == '5' || status == '6'){
      return '完成';
    }
    DateTime now = DateTime.now();
    var diff = time.millisecondsSinceEpoch - now.millisecondsSinceEpoch;
    if(diff <= 0)
      return "已超过规定时间";

    int dayDiff = (diff / 1000 / 3600 / 24).toInt();
    int hourDiff = ((diff - dayDiff*24*3600*1000) / 1000 / 3600).toInt();
    int minuteDiff = ((diff - dayDiff*24*3600*1000 - hourDiff * 3600*1000) / 1000 / 60).toInt();
    String des = "距预约开始还剩 ";
    if(dayDiff > 0) des += "$dayDiff天";
    if(hourDiff > 0) des += "$hourDiff小时";
    if(minuteDiff > 0) des += "$minuteDiff分";
    return des;
  }
