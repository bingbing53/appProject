...
typedef void OnDragChange(int nowIndex);

typedef bool CanAccept(int oldIndex, int newIndex);

BehaviorSubject<int> _tree = BehaviorSubject<int>();
Sink<int> get _treeSink => _tree.sink;
Stream<int> get treeStream => _tree.stream;
...
//  ScrollController scrollController = new ScrollController();
  ...
  void _maybeScroll() {
    ...
    if (shouldScrollUp) {
      ...
      isScrolling = true;
      var scrollTo = widget.scrollController.offset - 12;
      widget.scrollController
          .animateTo(scrollTo, duration: new Duration(milliseconds: 50), curve: Curves.linear)
          .then((it) {
        updatePlaceholder();
        isScrolling = false;
        _maybeScroll();
...
//          rows.removeAt(index);
  ...
  void _accept(int index, Data data) {
    ...
    setState(() {
      ...
      data.extraBot = 0.0;
      //TODO
      if (_currentMiddle.dy + 5 >= _currentScrollPos) {
...
class DraggableListItem extends StatelessWidget {
  ...
  final double _kScrollThreashhold = 0.0;
  ...
  Widget _getFeedback(int index, BuildContext context) {
    ...
    return new StreamBuilder(
        ...
        AsyncSnapshot<int> snapshot) {
          ...
          String startTime = '${hour < 10 ? '0$hour':hour}:${minute < 10 ? '0$minute':minute}';
          DateTime date = DateUtil.getDateTime('2019-09-09 $startTime');

          date = date.add(Duration(minutes: 60));
          ...
          return Container(
            ...
            child: //Text('$hour:$minute'),
              Material(
                child: Text('$startTime - ${date.hour<10?'0${date.hour}':date.hour}:${date.minute<10?'0${date.minute}':date.minute}',style: TextStyle(color: Colors.white,fontSize: Dimens.font_sp16),),
