  ...
  >
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <!-- 选择机构 -->
        <a-form-item label="机构分配" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select mode="multiple" style="width: 100%" placeholder="请选择机构" v-model="selectedOrgan">
            <a-select-option
              v-for="(organ,organindex) in OrganList"
              :key="organindex.toString()"
              :value="organ.id"
            >{{organ.postName}}
            </a-select-option>
          </a-select>
        ...
        <!--部门分配-->
        <template v-if="departDisabled == false">
...
import { addUser, editUser, queryUserRole,querySysRole, queryall, checkUsername, queryUserOrgan } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    initialRoleList() {
      queryall().then(res => {
        if (res.success) {
          console.log('角色用户：' + JSON.stringify(res))
          this.roleList = res.result
        } else {
          console.log(res.message)
        }
      })
    queryUserOrgan({ pageNo: 1, pageSize: 20 }).then(res => {
        console.log("机构分配："+JSON.stringify(res.result.records))
      if (res.success) {
        this.OrganList = res.result.records
      } else {
        console.log(res.message)
      }
    })
    ...
    loadUserRoles(userid) {
      queryUserRole({ userid: userid }).then(res => {
        console.log('userid为参数:' + JSON.stringify(res))
        if (res.success) {
    ...
    loadUserOrgan(userid) {
        querySysRole({ userid: userid }).then(res => {
          console.log('userid为参数222:' + JSON.stringify(res))
          if (res.success) {
            this.selectedOrgan = res.result
          } else {
            console.log(res.message)
          }
        })
    ...
    edit(record) {
      ...
      that.$nextTick(() => {
        that.form.setFieldsValue(pick(this.model, 'username', 'sex', 'realname', 'email', 'phone', 'activitiSync','post'))
      })
      // 调用查询用户对应的部门信息的方法
    ...
    moment,
    handleSubmit() {
      console.log('点击用户录入的提交按钮：' + this.roleDisabled, this.departDisabled)
      // return;
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          formData.selecteddeparts =
            (!!this.userDepartModel.departIdList && this.userDepartModel.departIdList.length > 0) ? this.userDepartModel.departIdList.join(',') : ''
    ...
    // 搜索用户对应的部门API
    onSearch() {
      console.log(this.$refs.departWindow.add());
