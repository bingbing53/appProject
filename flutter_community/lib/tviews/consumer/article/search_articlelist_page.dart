...
import 'package:helloworld/tviews/widget/gzx_search_article_result_list_widget.dart';
import 'package:helloworld/tviews/widget/gzx_searchresult_gridview_widget.dart';
import 'package:helloworld/tviews/widget/gzx_searchresult_list_widget.dart';
...
class SearchArticleResultListPage<T extends ScrollNotification> extends StatefulWidget {
  ...
  SearchArticleResultListPage(this.keyword,
      {this.isList = false,
        this.onTapfilter,
  ...
  @override
  State<StatefulWidget> createState() => SearchArticleResultListState();
}

class SearchArticleResultListState extends State<SearchArticleResultListPage>
    with AutomaticKeepAliveClientMixin<SearchArticleResultListPage>, SingleTickerProviderStateMixin {
  SearchArticleResultListModal listData = SearchArticleResultListModal(null);
  int page = 0;
  bool _isList;
  ...
  SearchArticleResultListState();
  MainBloc _mainBloc;
  @override
  void initState() {
//    WidgetsBinding.instance.addPostFrameCallback(_afterLayout);
    _mainBloc =  BlocProvider.of<MainBloc>(context);
    _mainBloc.onRefresh(labelId:'searchMall');
...
//        : GZXSearchResultGridViewWidget(listData, getNextPage: () => getSearchList(widget.keyword));
    ...
    var resultWidget = GZXSearchArticleResultListWidget(listData, getNextPage: () => getSearchList(widget.keyword));
...
//                height: animation.value,
  ...
  void getSearchList(String keyword) async {
    var data = await _mainBloc.getSearchArticleList(keyword,page++);

//    var data = await getSearchResult(keyword, page++);
    SearchArticleResultListModal list = SearchArticleResultListModal.fromArticleJson(data);
    if (mounted) {
      setState(() {
        if(listData.data == null) listData.data = [];
