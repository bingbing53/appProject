...
class Util {
  static int minute = 60;// 1分钟
  static int hour = 60 * minute;// 1小时
  static int day = 24 * hour;// 1天
  static int month = 31 * day;// 月
  static int year = 12 * month;// 年
  static String getTimeFormatText(String comTime){
    if(comTime == null || comTime.length <= 0)
      return "";
    var nowTime = DateTime.now().millisecondsSinceEpoch/1000;
    var compareTime = DateTime.parse(comTime).millisecondsSinceEpoch/1000;
    var diff = nowTime-compareTime;
    var r = 0.0;
    if (diff > year) {
      return comTime;
    }
