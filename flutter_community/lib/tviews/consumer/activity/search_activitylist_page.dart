...
import 'package:helloworld/tviews/widget/gzx_search_activity_result_list_widget.dart';
...
class SearchActivityResultListPage<T extends ScrollNotification> extends StatefulWidget {
  ...
  SearchActivityResultListPage(this.keyword,
      {this.isList = false,
        this.onTapfilter,
  ...
  @override
  State<StatefulWidget> createState() => SearchActivityResultListState();
}

class SearchActivityResultListState extends State<SearchActivityResultListPage>
    with AutomaticKeepAliveClientMixin<SearchActivityResultListPage>, SingleTickerProviderStateMixin {
  SearchActivityResultListModal listData = SearchActivityResultListModal(null);
  int page = 0;
  bool _isList;
...
//        : GZXSearchResultGridViewWidget(listData, getNextPage: () => getSearchList(widget.keyword));
    var resultWidget = GZXSearchActivityResultListWidget(listData, getNextPage: () => getSearchList(widget.keyword));
...
//                height: animation.value,
  ...
  void getSearchList(String keyword) async {
    var data = await _mainBloc.getSearchActivityList(keyword,page++);
    SearchActivityResultListModal list = SearchActivityResultListModal.fromActivityJson(data);
    if (mounted) {
      setState(() {
