        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.fileList.length; i++) {
                ...
                if(fileSrcFieldKeys.length > 1) {
                    ...
                    fileSrc = data.fullUrlPrefix + fileSrc
                }
                link = fileSrc;
                list.push({
                    title: data.original || link.substr(link.lastIndexOf('/') + 1),
                    url: microFileType(prefix + link)
                });
            }
            return list;
        }
    };
    function microFileType(link){
        var index= link.lastIndexOf(".");
        var ext = link.substr(index+1);
        if(['doc','docx','ppt','pptx','xls','xlsx','zip','rar','mp4','mp3','txt','html','xml','java','properties','sql','js','md','json','conf','ini','vue','php','py','bat','gitignore','pdf'].indexOf(ext.toLowerCase()) !== -1)
            return 'https://image.shequyijia.cn/onlinePreview?url='+encodeURIComponent(link);
