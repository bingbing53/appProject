...
class SearchResultListState extends State<SearchResultListPage>
  ...
  SearchResultListModal listData = SearchResultListModal(null);
  int page = 1;
  bool _isList;
                  ...
                  Expanded(
                    child: Container(
                      padding: EdgeInsets.only(bottom: 10),
                        color: _isList ? Colors.white : Colours.mainBackgroundColor,
...
//                height: animation.value,
  ...
  void getSearchList(String keyword) async {
    var data = await _mainBloc.getMallProductFromEs(keyword,page++);
    SearchResultListModal list = SearchResultListModal.fromMallJson(data);
    if (mounted) {
      setState(() {
        if(listData.mallData == null) listData.mallData = [];
        listData.mallData.addAll(list.mallData);
      });
    }
  }
