        ...
        child: Row(
            children: <Widget>[
              Container(
                ...
                child: ClipOval(
                    child:CachedNetworkImage(imageUrl: ShequejiaApi.format(message.image))
                ),
              ),
              Expanded(
          ...
          _children.add(Row(
            children: <Widget>[
              ...
              Container(
                  width: 60.0,
                  height: 60.0,
                  child: CachedNetworkImage(imageUrl: ShequejiaApi.format(message.image,size: 0))
              ),
            ],
          ));
          padding = Dimens.gap_dp10;
        }else {
          _children.add(CachedNetworkImage(imageUrl: ShequejiaApi.format(message.image,size: 0)));
...
//                NavigatorUtil.pushWeb(context,title: message.title,url: message.url);
//              }else if(ObjectUtil.isNotEmpty(message.rid) && ObjectUtil.isNotEmpty(message.prefix)){
//                NavigatorUtil.pushWeb(context,title: message.title,url: message.prefix+message.rid);
//              }
              if(ObjectUtil.isNotEmpty(message.prefix) && ObjectUtil.isNotEmpty(message.rid)){
                NavigatorUtil.pushWeb(context,title:message.data,url: '${message.prefix}${message.rid}');
      ...
      return Column(
        ...
        children: <Widget>[
          ...
          GestureDetector(
            ...
            onTap: (){
              NavigatorUtil.pushPage(context, WebScaffold(url:'${message.prefix}${message.rid}',title: '详情',hideTitleBar: message.action == 'mall'),pageName:'查看详情');
            },
          )
        ],
      );
