    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item
          ...
          <a-select v-model="model.modules" :defaultValue="model.modules" style="min-width: 120px">
            <a-select-option v-for="item in moduleList" :key="item.code">{{item.name}}</a-select-option>
          </a-select>
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="是否外部系统">
          <a-switch checkedChildren="是" unCheckedChildren="否" v-model="externalSwitth"/>
        </a-form-item>
...
<script>
  import {addPermission,editPermission,queryTreeList,queryModulesList} from '@/api/api'
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          modules:{rules:[{required:true,message:'请选择所属板块'}]},
    ...
    methods: {
      ...
      loadModulesData(){
        var that = this;
        queryModulesList().then((res)=>{
          if(res.success){
            that.moduleList = res.result.records;
          }
        });
      },
      add () {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.model.external = this.externalSwitth;
