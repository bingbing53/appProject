...
import 'package:helloworld/tviews/widget/message_chat_item/event_message.dart';
import 'package:helloworld/tviews/widget/simple_dialog.dart';
import 'package:helloworld/utils/util_index.dart';
import 'package:jmessage_flutter/jmessage_flutter.dart';

import 'message_chat_item/custom_message.dart';
import 'message_chat_item/file_message.dart';
import 'message_chat_item/image_message.dart';
import 'message_chat_item/location_message.dart';
import 'message_chat_item/text_message.dart';
import 'message_chat_item/voice_message.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if(model is JMEventMessage || model == null){
      ...
      if(model.extras != null && model.extras.length > 0){
        if(model.extras['msg_type'] == null || model.extras['msg_type'] != 'normal')
          return CustomMessage(model:model,bloc:bloc);
      }
    }
    //普通的聊天消息
    ...
    String portrait = ShequejiaApi.format(jmUserInfo.extras['portrait']); //头像
    String createTime = Utils.timeShortFormat(DateUtil.getDateStrByMs(model.createTime));
...
//            ),
  ...
  Widget buildWidget(){
    ...
    if(model is JMImageMessage){
      ...
      return TextMessage(model:model);
