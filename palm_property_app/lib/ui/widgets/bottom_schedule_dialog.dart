 ...
 */
class BottomScheduleDialog extends StatelessWidget {

  final SysUserModel sysUserModel;

  const BottomScheduleDialog({Key key, this.sysUserModel}) : super(key: key);
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Container(
                    ...
                    child:Row(
                      children: <Widget>[
//                        Text(sysUserModel.realname,textAlign: TextAlign.start,),
//                        SizedBox(width: Dimens.gap_dp15,),
                  ...
                  Container(
                      height:435,
                      color: Colors.white,
                      child:TimeSchedulePage(bloc:bloc,sysUserModel:sysUserModel)
                  ),
...
class TimeSchedulePage extends StatelessWidget{
  ...
  const TimeSchedulePage({Key key, this.bloc,this.sysUserModel}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    bloc.getTimeSchedule(sysUserModel.id);
    ...
    for(int i = 0;i < 7; i++){
      ...
      while(nowDate.hour <= 21){
        ...
        for(int j = 0;j<times.length;j++){
          if(times[j].startTime == nowDate){
            enabled = false;
            break;
          }
        }
        TimeSelectedModel timeSelectedModel = new TimeSelectedModel(nowDate,i,enabled,false);
